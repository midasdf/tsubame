...
pub fn isBookmarked(db: *storage.Database, url: [*:0]const u8) bool {
    const stmt = db.prepare("SELECT id FROM bookmarks WHERE url = ?") catch return false;
    defer _ = c.sqlite3_finalize(stmt);
    _ = c.sqlite3_bind_text(stmt, 1, url, -1, null);
    return c.sqlite3_step(stmt) == c.SQLITE_ROW;
}

pub fn addBookmark(db: *storage.Database, url: [*:0]const u8, title: ?[*:0]const u8) void {
...
pub fn removeBookmark(db: *storage.Database, url: [*:0]const u8) void {
    const stmt = db.prepare("DELETE FROM bookmarks WHERE url = ?") catch return;
    defer _ = c.sqlite3_finalize(stmt);
    _ = c.sqlite3_bind_text(stmt, 1, url, -1, null);
    _ = c.sqlite3_step(stmt);
}

pub fn toggleBookmark(db: *storage.Database, url: [*:0]const u8, title: ?[*:0]const u8) bool {
    if (isBookmarked(db, url)) {
        removeBookmark(db, url);
        return false;
    } else {
        addBookmark(db, url, title);
        return true;
    }
}

pub fn generateBookmarksPage(db: *storage.Database, allocator: std.mem.Allocator) ?[]const u8 {
    const stmt = db.prepare(
